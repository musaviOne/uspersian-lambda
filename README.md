...
```bash
sam local invoke UsPersianFunction --event event.json
```
...
aws cloudformation describe-stacks \
    --stack-name sam-app \
    --query 'Stacks[].Outputs[?OutputKey==`UsPersianApi`]' \
    --output table
``` 
...
```bash
sam logs -n UsPersianFunction --stack-name sam-app --tail
```
...
* Invoke with ``sam local invoke UsPersianFunction --event event.json``
* Update tests
